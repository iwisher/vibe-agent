...
BUILTIN_PATTERNS: list[PatternDef] = [
    # ── CRITICAL: auto-block ───────────────────────────────────────────────
    {
        "id": "rm-rf-root",
        "severity": "critical",
        "pattern": r"rm\s+-[a-zA-Z]*f.*\s+/\s*$|rm\s+-[a-zA-Z]*f\s+/\s",
        "description": "rm -rf / or similar",
    },
    {
        "id": "rm-rf-home",
        "severity": "critical",
    ...
    },
    {
        ...
        "description": "dd overwrite disk with zeros",
    },
    {
        "id": "dd-dev-random",
        "severity": "critical",
        "pattern": r"dd\s+.*if=/dev/urandom\s+.*of=/dev/[sh]d[a-z]",
        "description": "dd overwrite disk with random",
    },
    {
    ...
    },
    {
        "id": "shutdown-now",
        "severity": "critical",
        "pattern": r"\bshutdown\s+-h\s+now\b|\bpoweroff\b|\breboot\b",
        "description": "System shutdown/reboot",
    },
    {
        "id": "init-sysrq",
        "severity": "critical",
        "pattern": r"echo\s+[a-z]\s*>\s*/proc/sysrq-trigger",
    ...
    },
    {
        ...
        "description": "Flush all iptables rules",
    },
    {
        "id": "userdel-root",
        "severity": "critical",
        "pattern": r"userdel\s+root",
    ...
    },
    {
        "id": "wget-pipe-sh",
        "severity": "critical",
        "pattern": r"wget\s+.*\|\s*(sh|bash)\b",
        "description": "wget | sh pipe",
    },
    {
        "id": "curl-pipe-sh",
        "severity": "critical",
    ...
    # ── WARNING: flag for review ───────────────────────────────────────────
    {
        ...
        "description": "Git hard reset (destructive)",
    },
    {
        "id": "git-force-push",
        "severity": "warning",
        "pattern": r"git\s+push\s+.*--force\b|\bgit\s+push\s+.*-f\b",
        "description": "Git force push",
    },
    {
        "id": "git-clean-dfx",
        "severity": "warning",
        "pattern": r"git\s+clean\s+-[a-z]*[df]",
    ...
    },
    {
        "id": "sudo-no-pwd",
        "severity": "warning",
        "pattern": r"\bsudo\b(?!\s+-S)",
        "description": "sudo without -S (may hang)",
    },
    {
        "id": "docker-socket",
        "severity": "warning",
        "pattern": r"docker\s+.*-v\s+/var/run/docker\.sock",
        "description": "Docker socket mount (container escape)",
    },
    {
        "id": "docker-privileged",
        "severity": "warning",
        "pattern": r"docker\s+.*--privileged",
        "description": "Docker privileged mode",
    },
    {
        "id": "docker-host-pid",
        "severity": "warning",
        "pattern": r"docker\s+.*--pid\s+host",
        "description": "Docker host PID namespace",
    },
    {
        "id": "docker-host-net",
        "severity": "warning",
        "pattern": r"docker\s+.*--network\s+host",
        "description": "Docker host network",
    },
    {
    ...
    },
    {
        "id": "pip-install-user",
        "severity": "warning",
        "pattern": r"pip\s+install\s+.*--user",
        "description": "pip user install",
    },
    {
        "id": "rustup-default",
        "severity": "warning",
        "pattern": r"rustup\s+default",
        "description": "rustup default toolchain change",
    },
    {
        "id": "cargo-install",
        "severity": "warning",
        "pattern": r"cargo\s+install",
        "description": "cargo install (downloads from crates.io)",
    },
    {
        "id": "brew-install",
        "severity": "warning",
        "pattern": r"brew\s+(install|reinstall|upgrade)",
        "description": "brew install/upgrade",
    },
    {
        "id": "apt-install",
        "severity": "warning",
        "pattern": r"apt\s+(install|remove|purge)",
        "description": "apt package management",
    },
    {
        "id": "yum-install",
        "severity": "warning",
        "pattern": r"yum\s+(install|remove)",
        "description": "yum package management",
    },
    {
        "id": "pacman-syu",
        "severity": "warning",
        "pattern": r"pacman\s+-[a-z]*[SyU]",
        "description": "pacman system upgrade",
    },
    {
        "id": "systemctl-enable",
        "severity": "warning",
        "pattern": r"systemctl\s+(enable|disable|start|stop|restart)",
        "description": "systemctl service management",
    },
    {
        "id": "crontab-edit",
        "severity": "warning",
    ...
    },
    {
        "id": "ssh-keygen",
        "severity": "warning",
        "pattern": r"ssh-keygen",
        "description": "SSH key generation",
    },
    {
        "id": "openssl-genrsa",
        "severity": "warning",
        "pattern": r"openssl\s+genrsa",
        "description": "OpenSSL key generation",
    },
    {
        "id": "gpg-gen-key",
        "severity": "warning",
        "pattern": r"gpg\s+--gen-key|gpg\s+--full-generate-key",
        "description": "GPG key generation",
    },
    {
        "id": "wget-execute",
        "severity": "warning",
        "pattern": r"wget\s+.*\s+-O\s+-.*\|\s*(sh|bash|python)",
        "description": "wget output to pipe",
    },
    {
        "id": "curl-execute",
        "severity": "warning",
        "pattern": r"curl\s+.*\s+-o\s+-.*\|\s*(sh|bash|python)",
        "description": "curl output to pipe",
    },
    # ── INFO: log only ─────────────────────────────────────────────────────
    {
        "id": "sudo-with-s",
        ...
        "description": "sudo with -S (non-interactive)",
    },
    {
        "id": "eval-string",
        "severity": "info",
    ...
    },
    {
        "id": "source-remote",
        "severity": "info",
        "pattern": r"source\s+<(curl|wget)",
        "description": "source from remote URL",
    },
    {
        "id": "bash-c-url",
        "severity": "info",
        "pattern": r"bash\s+-c\s+['\"].*(curl|wget)",
        "description": "bash -c with curl/wget",
    },
    # ── Inline eval detection across interpreters ──────────────────────────
    {
        "id": "python-inline",
        "severity": "warning",
        "pattern": r"\bpython\d*\s+-[cm]\s+['\"]",
    ...
    },
    {
        ...
        "severity": "warning",
        "pattern": r"\bawk\s+['\"].*\{.*\}",
        "description": "AWK script execution",
    },
    # ── Wrapper detection ──────────────────────────────────────────────────
    {
        "id": "wrapper-sudo",
        "severity": "warning",
        "pattern": r"\bsudo\b",
        "description": "sudo wrapper detected",
    },
    {
        "id": "wrapper-doas",
        "severity": "warning",
        "pattern": r"\bdoas\b",
        "description": "doas wrapper detected",
    },
    {
        "id": "wrapper-chrt",
        "severity": "warning",
        "pattern": r"\bchrt\b",
        "description": "chrt wrapper detected",
    },
    {
        "id": "wrapper-ionice",
        "severity": "warning",
        "pattern": r"\bionice\b",
        "description": "ionice wrapper detected",
    },
    {
        "id": "wrapper-taskset",
        "severity": "warning",
        "pattern": r"\btaskset\b",
        "description": "taskset wrapper detected",
    },
    {
        "id": "wrapper-setsid",
        "severity": "warning",
        "pattern": r"\bsetsid\b",
        "description": "setsid wrapper detected",
    },
    {
        "id": "wrapper-env",
        "severity": "info",
        "pattern": r"\benv\s+\w+=",
        "description": "env wrapper (variable setting)",
    },
    {
        "id": "wrapper-nice",
        "severity": "info",
        "pattern": r"\bnice\s+-?\d*\s",
        "description": "nice wrapper (priority adjustment)",
    },
    {
        "id": "wrapper-timeout",
        "severity": "info",
        "pattern": r"\btimeout\s+\d+",
    ...
    # ── Data exfiltration / network ────────────────────────────────────────
    {
        "id": "nc-listen",
        "severity": "warning",
        "pattern": r"\bnc\s+.*-l\b|\bncat\s+.*-l\b|\bnetcat\s+.*-l\b",
        "description": "Netcat listener (backdoor)",
    },
    {
        "id": "nc-connect",
        "severity": "warning",
        "pattern": r"\bnc\s+\d+\.\d+\.\d+\.\d+\s+\d+\b",
        "description": "Netcat outbound connection",
    },
    {
        "id": "python-http-server",
        "severity": "warning",
        "pattern": r"python\d*\s+-m\s+http\.server",
        "description": "Python HTTP server",
    },
    {
        "id": "python-https-server",
        "severity": "warning",
        "pattern": r"python\d*\s+-m\s+http\.server.*--bind",
        "description": "Python HTTP server with bind",
    },
    {
        "id": "socat",
        "severity": "warning",
        "pattern": r"\bsocat\b",
        "description": "socat (arbitrary socket relay)",
    },
    {
        "id": "tcpdump",
        "severity": "warning",
        "pattern": r"\btcpdump\b",
    ...
    },
    {
        ...
        "severity": "warning",
        "pattern": r"\btshark\b",
        "description": "tshark (packet capture)",
    },
    {
        "id": "scp-outbound",
        "severity": "warning",
        "pattern": r"\bscp\s+.*\d+\.\d+\.\d+\.\d+",
        "description": "SCP to remote host",
    },
    {
        "id": "rsync-remote",
        "severity": "warning",
        "pattern": r"\brsync\s+.*\w+@\d+\.\d+\.\d+\.\d+",
    ...
    },
    {
        "id": "ftp-connect",
        "severity": "warning",
        "pattern": r"\bftp\s+\d+\.\d+\.\d+\.\d+",
        "description": "FTP connection",
    },
    {
        "id": "telnet-connect",
        "severity": "warning",
        "pattern": r"\btelnet\s+\d+\.\d+\.\d+\.\d+",
    ...
    },
    {
        "id": "cat-ssh-keys",
        "severity": "warning",
        "pattern": r"cat\s+.*\.ssh/(id_rsa|id_ed25519|id_ecdsa)",
        "description": "Read SSH private keys",
    },
    {
        "id": "cat-aws-creds",
        "severity": "warning",
        "pattern": r"cat\s+.*\.aws/credentials",
    ...
    },
    {
        "id": "env-secrets",
        "severity": "warning",
        "pattern": r"env\s*\|\s*grep\s+-i\s+(secret|password|token|key)",
        "description": "Grep env for secrets",
    },
    # ── Process / system manipulation ──────────────────────────────────────
    {
        ...
        "description": "killall (kill processes by name)",
    },
    {
        "id": "pkill",
        "severity": "warning",
        "pattern": r"\bpkill\b",
        "description": "pkill (kill processes by pattern)",
    },
    {
        "id": "kill-9",
        "severity": "warning",
    ...
    },
    {
        "id": "xargs-rm",
        "severity": "warning",
        "pattern": r"xargs\s+rm",
        "description": "xargs rm (bulk delete)",
    },
    {
        "id": "find-exec-rm",
        "severity": "warning",
        "pattern": r"find\s+.*-exec\s+rm",
    ...
    },
    {
        "id": "truncate",
        "severity": "warning",
        "pattern": r"\btruncate\s+-s\s+0\b",
        "description": "truncate to zero (data destruction)",
    },
    {
        "id": "shred",
        "severity": "critical",
        "pattern": r"\bshred\b",
        "description": "shred (secure delete)",
    },
    {
        "id": "wipefs",
        "severity": "critical",
        "pattern": r"\bwipefs\b",
        "description": "wipefs (filesystem signature erase)",
    },
]
